...
class ExpandedAchievementCard extends StatefulWidget {
  ...
  const ExpandedAchievementCard({Key key, this.achievement, this.onClose})
      : super(key: key);

  @override
  _ExpandedAchievementCardState createState() =>
      _ExpandedAchievementCardState();
}
...
class _ExpandedAchievementCardState extends State<ExpandedAchievementCard> {
  ...
  Widget build(BuildContext context) {
    ...
    int progressDenominator =
        widget.achievement.nextTarget - widget.achievement.previousTarget;
    progressDenominator = (progressDenominator == 0)
        ? widget.achievement.progressToTarget
        : progressDenominator;

    if (widget.achievement.type == AchievementType.Tiered) {
      achievementTypeWidget = Column(children: [
        Expanded(
          flex: 2,
          child: Center(
            child: Text(
              "Level : " + widget.achievement.level.toString(),
          ...
          child: Container(
            ...
            child: ClipRRect(
              ...
              child: LinearProgressIndicator(
                ...
                valueColor:
                    AlwaysStoppedAnimation<Color>(CustomColours.iconGreen()),
      ...
      achievementTypeWidget = Center(
        child: Text(
            (widget.achievement.achieved) ? "Achieved!" : "Not yet achieved",
            style: TextStyle(
    ...
    List<Expanded> achievementCardElements = [
      Expanded(
        ...
        child: (widget.achievement.type == AchievementType.Unlock)
            ? AchievementImages.retrieveOneTimeImage(widget.achievement.id)
            : AchievementImages.retrieveLevelImage(
                widget.achievement.id, widget.achievement.level),
      )
    ];
    achievementCardElements
        .add(Expanded(flex: 3, child: achievementTypeWidget));
